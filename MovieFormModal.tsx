...
export const MovieFormModal: React.FC<MovieFormModalProps> = ({ isVisible, onClose, movieToEdit }) => {
    const { insertMovie, updateMovie } = useMovies(); // Lấy hàm insertMovie
    ...
    useEffect(() => {
        if (movieToEdit) {
            ...
            setYear(movieToEdit.year ? String(movieToEdit.year) : '');
            setRating(movieToEdit.rating ? String(movieToEdit.rating) : '');
        } else {
            setTitle('');
            setYear('');
            setRating('');
    ...
    const handleSave = () => {
        ...
        if (movieToEdit) {
            const success = updateMovie(movieToEdit.id, title.trim(), yearNum, ratingNum);
            if (success) {
                Alert.alert('Thành công', 'Cập nhật phim thành công!');
                onClose();
            } else {
                Alert.alert('Lỗi', 'Không thể cập nhật phim.');
            }
        } else {
            // Logic INSERT (C4)
